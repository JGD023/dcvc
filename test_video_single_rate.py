...
from src.models.video_model_train import DMC
...
from src.utils.transforms import rgb2ycbcr, ycbcr2rgb, yuv_444_to_420, ycbcr420_to_444_np, rgb_to_ycbcr444, ycbcr444_to_420, ycbcr444_to_rgb
    ...
    with torch.no_grad():
        ...
        for frame_idx in range(frame_num):
            ...
            if frame_idx == 0 or (intra_period > 0 and frame_idx % intra_period == 0):
                ...
                encoded = i_frame_net.inference(x_padded, args['qp_i'])
                ...
                encoded = p_frame_net.inference(x_padded, fa_idx)
                recon_frame = encoded['recon_image']
    ...
    with torch.no_grad():
        ...
        for frame_idx in range(frame_num):
            ...
            if frame_idx == 0 or (intra_period > 0 and frame_idx % intra_period == 0):
                ...
                encoded = p_frame_net.compress(x_padded, fa_idx)
                last_qp = curr_qp
                frame_types.append(1)
    ...
    with torch.no_grad():
        while decoded_frame_number < frame_num:
            ...
            if header['nal_type'] == NalType.NAL_I:
                ...
                if sps['fa_idx'] == 3:
                    p_frame_net.reset_ref_feature()
                decoded = p_frame_net.decompress(bit_stream, sps)
